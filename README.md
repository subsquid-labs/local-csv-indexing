...
We recommend that you read Subsquid docs to understand how it works: https://docs.sqd.dev/en/sdk
...
* The main logic of this project is defined in `src/processor.ts`. The `EvmBatchProcessor` class is configured and used to perform request to [Subsquid's Archive for Ethereum blockchain](https://cloud.sqd.dev), to obtain necessary data. Then some custom logic is implemented to process this data in batches, and save it on the database with the custom defined structure.

[Subsquid documentation](https://docs.sqd.dev/en/sdk) has dedicated sections and pages describing each of these concepts, it is advised to consult them, before starting to develop your own squid.
...
See [docs on schema updates](https://docs.sqd.dev/en/sdk) for more details.
...
For more details about `squid-evm-typegen` read the [docs page](https://docs.sqd.dev/en/sdk)
...
[type-graphql](https://typegraphql.com) resolvers and to add request validation. See [the docs](https://docs.sqd.dev/en/sdk) for more details.
