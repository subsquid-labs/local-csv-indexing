...
import { CsvDatabase, Table, TableRecord, types } from "@subsquid/csv-store";
...
const Gravatars = new Table('gravatars', {
  id: types.string,
  owner: types.string,
...
const db = new CsvDatabase({
  tables: [Gravatars],
  dest: `${__dirname}/../assets/gravatars-table`,
  chunkSizeMb: 10,
  syncIntervalBlocks: 100_000,
})

type Record = TableRecord<typeof Gravatars>;

processor.run(db, async (ctx) => {
    const gravatars: Record[] = [];
    ...
    ctx.store.write(Gravatars, gravatars);
});
